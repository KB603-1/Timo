  ...
  <div class="space-y-5">
    ...
    <RadioGroup v-model="selectedType" class="grid grid-cols-2 gap-2">
      <Label
          ...
          :class="selectedType === 'income' ? 'border-emerald-500 bg-emerald-50 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-400' : 'hover:bg-muted'"
      >
      ...
      <Label
          for="expense"
          class="flex items-center justify-center gap-2 rounded-lg border px-4 py-3 cursor-pointer transition-colors"
          :class="selectedType === 'expense' ? 'border-rose-500 bg-rose-50 text-rose-700 dark:bg-rose-950 dark:text-rose-400' : 'hover:bg-muted'"
      >
    ...
    <div class="space-y-1.5">
      ...
      <Input id="title" v-model="title" type="text" placeholder="예) 스타벅스 아메리카노"/>
    </div>
    ...
    <div class="space-y-1.5">
      <Label for="memo" class="text-sm font-medium">
        메모
        <span class="ml-1 text-xs text-muted-foreground font-normal">(선택)</span>
      </Label>
      <Textarea id="memo" v-model="memo" placeholder="메모를 입력해주세요" class="resize-none" rows="3"/>
    </div>
